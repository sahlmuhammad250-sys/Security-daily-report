...
function SeksiAktivitas({ rows, setRows }: { rows:PatroliRow[]; setRows:(r:PatroliRow[])=>void }) {
  ...
  return (
    <SCard title="C. Catatan Patroli" icon={<Activity size={16}/>} count={rows.length}>
      ...
      <BtnAdd onClick={()=>setRows([...rows,mkPatroli()])}/>
    </SCard>
  );
}
...
function RiwayatModal({ onClose, onCountChange, onCetakUlang }: { onClose: ()=>void; onCountChange: (n:number)=>void; onCetakUlang: (data: LaporanData)=>void }) {
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f4f6f9]" style={{fontFamily:"'Inter',sans-serif"}}>
      ...
      {showRiwayat&&<RiwayatModal onClose={()=>setShowRiwayat(false)} onCountChange={setRiwayatCount} onCetakUlang={handleCetakUlang}/>}
    </div>
  );
}
